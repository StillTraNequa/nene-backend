...
app.post('/create-checkout-session', async (req, res) => {
  ...
  // Validate deposit guest count (2–5)
  if (isDeposit) {
    const guests = parseInt(meta?.guests, 10);
    if (!guests || guests < 2 || guests > 5) {
      return res.status(400).json({ error: 'Invalid guests count for deposit (must be 2–5).' });
    }
  }
  ...
  transporter.sendMail(
    {
      ...
      subject: 'Order Confirmation - NeNe Nail’d It',
      text: `
